...
2. Determine the gate being bypassed:
   - Run `Bash("git diff --cached")` and capture the output.
     - If non-empty → this is a **code** gate bypass. Compute `sha256` of the diff; call that `target_hash`.
   - Else if the state file has `pending_review == "plan"` → this is a **plan** gate bypass. Use `state.pending_hash` as `target_hash`.
...
3. Update the state file:
   ...
   - Clear `pending_review` and `pending_hash`.
   - Append to `bypass_log` (create the array if missing) an entry:
     ```json
     { "ts": "<iso>", "gate": "plan|code", "hash": "<target_hash>", "reason": "<user reason from $ARGUMENTS>" }
     ```
...
- Never bypass both gates in one invocation. If both appear pending, ask the user which to bypass.
- Never clear `bypass_log`. Entries are append-only.
- The bypass applies only to the current SHA. Any further change re-arms the gate.
